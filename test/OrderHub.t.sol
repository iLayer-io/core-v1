...
contract OrderHubTest is BaseTest {
    ...
    /**
     * @notice Test creation of an order with an ERC1155 batch transfer.
     */
    function testCreateERC1155BatchOrder() public {
        ...
        Validator.Order memory order = buildERC1155BatchOrder(
            user0,
            address(this),
        ...
        vm.prank(user0);
        inputERC1155Token.mintBatch(user0, ids, amounts, "");

        vm.prank(user0);
        inputERC1155Token.setApprovalForAll(address(hub), true);

        assertEq(inputERC1155Token.balanceOf(user0, ids[0]), amounts[0]);
        assertEq(inputERC1155Token.balanceOf(user0, ids[1]), amounts[1]);
        assertEq(inputERC1155Token.balanceOf(address(hub), ids[0]), 0);
        assertEq(inputERC1155Token.balanceOf(address(hub), ids[1]), 0);

        vm.expectEmit(true, true, true, true);
        emit ERC1155BatchReceived(address(hub), user0, ids, amounts, "");
        hub.createOrder(buildOrderRequest(order, 1), permits2, signature);

        assertEq(inputERC1155Token.balanceOf(user0, ids[0]), 0);
        assertEq(inputERC1155Token.balanceOf(user0, ids[1]), 0);
        assertEq(inputERC1155Token.balanceOf(address(hub), ids[0]), amounts[0]);
        assertEq(inputERC1155Token.balanceOf(address(hub), ids[1]), amounts[1]);
    }
}
